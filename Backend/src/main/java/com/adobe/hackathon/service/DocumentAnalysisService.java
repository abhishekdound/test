...
// Save final results
    ...
    private DetailedAnalysisResponse createDetailedResponse(Map<String, Object> pdfAnalysis,
        ...
        // Extract sections with importance ranking using Python-inspired logic
        List<ExtractedSection> extractedSections = pythonLogicSectionExtractionService
                .extractTopHeadings(jobDirectory, persona, jobToBeDone);
        detailedResponse.setExtractedSections(extractedSections);

        // Extract subsection analysis (refined text under those headings)
        List<SubsectionAnalysis> subsectionAnalysis = pythonLogicSectionExtractionService
    ...
    // Enhanced analysis support using Python logic
    public EnhancedDetailedAnalysisResponse createEnhancedResponseWithPythonLogic(
        ...
        try {
            // Extract detailed sections using Python logic
            List<DetailedExtractedSection> detailedSections = extractDetailedSectionsWithPythonLogic(
                    jobDirectory, persona, jobToBeDone);

            // Extract detailed subsections using Python logic
            List<DetailedSubsectionAnalysis> detailedSubsections = extractDetailedSubsectionsWithPythonLogic(
                    jobDirectory, detailedSections.stream().limit(15).collect(Collectors.toList()));

            // Create enhanced response
            return createEnhancedResponse(pdfAnalysis, jobDirectory, persona, jobToBeDone,
                    detailedSections, detailedSubsections, processingStartTime, submissionTime);

        } catch (Exception e) {
            logger.error("Error creating enhanced response with Python logic", e);
            throw new RuntimeException("Failed to create enhanced response: " + e.getMessage());
        }
    }
    ...
    // Enhanced analysis support methods
    public EnhancedDetailedAnalysisResponse createEnhancedResponse(
            Map<String, Object> pdfAnalysis, String jobDirectory, String persona, String jobToBeDone,
    ...
    private List<EnhancedDetailedAnalysisResponse.EnhancedMetadata.DocumentInfo> createDocumentInfoList(
        ...
        if (pdfAnalysis.containsKey("files")) {
            ...
            for (Map<String, Object> file : files) {
                ...
                docInfo.setCreationDate(java.time.LocalDate.now().toString());
